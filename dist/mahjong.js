...
(function (ClaimType) {
    ...
    ClaimType[ClaimType["ConcealedKong"] = 4] = "ConcealedKong";
...
function hasPoint(player) {
    ...
    // 判断花
    if (player.flowerTiles.length && undefined(player.flowerTiles, function (tile) { return (tile - 50) % 4 === point; }) > -1) {
    ...
    var groups = undefined(player.handTiles, function (tile) {
        return tile / 1;
    });
    // 对子是中、发、白，需要番
    if (!needPoint) {
    ...
    var _loop_1 = function (i, len) {
        ...
        if (item >= 2) {
            ...
            newSizes.forEach(function (item) {
                ...
                if (s % 3 !== 0 || undefined(assembly[s], i) === -1) {
                    flag_1 = false;
                }
            });
            if (flag_1) {
                eye = undefined(Array(2), uniqTiles[i]);
                return "break";
            }
        }
    };
...
function groupSize(tiles) {
    var groups = groupBy(tiles);
    var sides = [];
    undefined(groups).forEach(function (group) {
...
function groupBy(tiles) {
    return undefined(tiles, function (tile) {
        return tile / 1;
    });
}
...
function getSequence(tiles, tile) {
    var melds = [];
    tiles = undefined(undefined(tiles, [tile]));
    // 将牌组组成3个一起，然后判断是否是顺子
    if (tiles.length >= 2) {
        var sequences = [];
